...
bool job_shop::v_receiving_traverse_machine::judge_receiver_task_phase() {
    ...
    if (this->receiving_task_id_now <= 5) {
        ...
        }else if (this->receiving_t_now == receiving_task_id_time[this->receiving_task_id_now]) {//接车横移机回到4车道（起点）----空闲
    ...
    //当前任务为7-12个，取返回道的车的车放入某车道
        ...
        }else if (this->receiving_t_now == receiving_task_id_time[this->receiving_task_id_now]/2) {//接车横移机到达要放置车身车道
            std::cout<<"到达"<<this->receiving_task_id_now + 1 <<"车道"<<std::endl;
            //TODO 放置车身，更新车道信息

        }else if (this->receiving_t_now == receiving_task_id_time[this->receiving_task_id_now]){//接车横移机返回四车道（起点）
